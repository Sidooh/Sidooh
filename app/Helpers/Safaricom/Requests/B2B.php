...
class B2B extends MpesaClient
{
    ...
    public function __construct()
    {
        ...
        $this->partyA = config('mpesa.initiator.short_code');
        $this->senderIdentifierType = $this->identifier[config('mpesa.initiator.type')];

        $this->queueTimeOutURL = $this->setUrl(config('mpesa.queue_timeout_url.b2b'));
        $this->resultURL = $this->setUrl(config('mpesa.result_url.b2b'));
    }
    ...
    protected function pay($business, $amount, $remarks, $accountReference = '')
    {
        $parameters = [
            ...
            'AccountReference' => Str::limit($accountReference, 20, ''),
