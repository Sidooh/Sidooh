...
@section('content')
    ...
    <div class="card mb-3">
        ...
        <div class="card-body position-relative">
            <h5>UserNotification Details: #{{ $userNotification->id }}</h5>
            <p class="fs--1">{{ local_date($userNotification->created_at, 'M d, Y, h:m A') }}</p>
                    ...
                    <div class="badge rounded-pill badge-soft-success fs--2">
                        ...
                        @else
                            <div class="badge rounded-pill badge-soft-warning fs--2">
                                ...
                                @endif
                                {{ $userNotification->active ? 'active' : 'inactive' }}
                            </div>
                    </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-4 mb-4 mb-lg-0">
                        <h5 class="mb-3 fs-0">UserNotification</h5>
                        <h6 class="mb-2">
                            <a
                                href="tel:{{ $userNotification->phone }}">{{ $userNotification->phone }}</a>
                        </h6>
                        {{--                            <p class="mb-1 fs--1">2393 Main Avenue<br/>Penasauka, New Jersey 87896</p>--}}
                        <p class="mb-0 fs--1"><strong>Email: </strong><a
                                href="mailto:{{ $userNotification->user ? $userNotification->user->email : null }}">{{ $userNotification->user ? $userNotification->user->email : null }}</a>
                        </p>
                    ...
                    <div class="col-lg-4 mb-4 mb-lg-0">
                        ...
                        <h6 class="mb-2">
                            <a href="{{ $userNotification->user ? route('admin.users.show', $userNotification->user ) : route('admin.userNotifications.show', $userNotification ) }}">
                                {{ $userNotification->user ? $userNotification->user->name : $userNotification->phone }}
                            </a>
                        </h6>
                        <p class="mb-0 fs--1"><strong>ID
                                Number: </strong>{{ $userNotification->user ? $userNotification->user->id_number : null }}
                        </p>
                        @if($userNotification->user)
                            <div class="fs--1"><strong>Status: </strong>
                                @if($userNotification->user->status == 'active')
                                    <div class="badge rounded-pill badge-soft-success fs--2">
                                        <span class="ms-1 fas fa-check" data-fa-transform="shrink-2"></span>
                                        {{ $userNotification->user->status }}
                                    </div>
                                @else
                                    <div class="badge rounded-pill badge-soft-warning fs--2">
                                        <span class="ms-1 fas fa-stream" data-fa-transform="shrink-2"></span>
                    ...
                    <div class="col-lg-4">
                        ...
                        <div class="d-flex">
                            @if($userNotification->referrer)
                                <div class="flex-1">
                                    <h6 class="mb-0">
                                        <a href="{{ route('admin.userNotifications.show', $userNotification->referrer ) }}">
                                            {{ $userNotification->referrer->phone }}
                                        </a>
                                    </h6>
                                    @if($userNotification->referrer->user)
                                        <p class="mb-0 fs--1">
                                            <strong>User: </strong>
                                            <a href="{{ route('admin.users.show', $userNotification->referrer->user ) }}">
                                                {{ $userNotification->referrer->user->name }}
                                            </a>
                                        </p>
                                    @endif
                                    <p class="mb-0 fs--1">
                                        <strong>Status: </strong>
                                    @if($userNotification->referrer->active)
                                        <div class="badge rounded-pill badge-soft-success fs--2">
                                            <span class="ms-1 fas fa-check" data-fa-transform="shrink-2"></span>
                                            active
                                        </div>
            ...
            <div class="col-sm-6 col-md-4">
                <div class="card overflow-hidden" style="min-width: 12rem">
                    ...
                    <div class="card-body position-relative">
                        ...
                        {{--                                <a class="fw-semi-bold fs--1 text-nowrap" href="{{ route('admin.userNotifications.index') }}">See all<span--}}
        ...
        @if(count($userNotification->level_referrals))
            <div class="card mb-3">
                <div class="card-body">
                    <h6>Referrals</h6>
                    <div class="table-responsive fs--1">
                        <table class="table table-striped border-bottom">
                            ...
                            <tr>
                                ...
                                <th class="border-0">UserNotifications</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach($userNotification->level_referrals as $level => $referrals)
                                <tr class="border-200">
                                    ...
                                    <td class="align-middle">
                                        ...
                                        @foreach($referrals as $referral)
                                            <a href="{{ route('admin.userNotifications.show', $referral) }}">
                                                    <span
                            ...
                            <tr class="border-200 border-top-2">
                                <td colspan="3" class="align-middle text-center">Total
                                    Referrals: {{ $userNotification->total_level_referrals }}</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="card h-100 bg-line-chart-gradient">
                    <div class="card-header bg-transparent light">
                        ...
                        <div class="real-time-user display-3 fw-normal text-white"
                             data-countup='{"endValue":{{ $userNotification->voucher->balance }},"prefix":"KES ","decimalPlaces":"0"}'>
                            0
                        </div>
                    </div>
                </div>
            </div>
            @foreach($userNotification->sub_userNotifications as $acc)
                <div class="col-md-4 col-lg-3 pe-lg-2">
                    <div class="card h-100 bg-line-chart-gradient">
                        <div class="card-header bg-transparent light">
