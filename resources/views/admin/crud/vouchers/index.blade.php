...
@section('content')
    ...
    <div class="card mb-3" id="customersTable"
         data-list='{"valueNames":["account","in","out","balance","created","updated"],"page":10,"pagination":true}'>
        <div class="card-header">
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Vouchers</h5>
                </div>
                    ...
                    @foreach($vouchers as $voucher)
                        ...
                        <tr class="btn-reveal-trigger">
                            ...
                            <td class="phone align-middle white-space-nowrap py-2">
                                ...
                                <a href="{{ route('admin.accounts.show', $voucher->account ) }}">{{ $voucher->account->phone }}</a>

                                @if($voucher->account->user)
                                    <a href="{{ route('admin.users.show', $voucher->account->user) }}">
                                        <div class="d-flex d-flex align-items-center">
                                            <div class="flex-1">
                                                <h5 class="mb-0 fs--1">{{ $voucher->account->user->name }}</h5>
                                            </div>
                                        </div>
                                    </a>
                                @endif
                            ...
                            <td class="name align-middle white-space-nowrap py-2">
                                {{ format_cur($voucher->in) }}
                            </td>
                            <td class="name align-middle white-space-nowrap py-2">
                                {{ format_cur($voucher->out) }}
                            </td>
                            <td class="name align-middle white-space-nowrap py-2">
                                {{ format_cur($voucher->balance) }}
                            </td>

                            <td class="joined align-middle py-2">{{ local_date($voucher->created_at, 'd/m/Y H:i') }}</td>
                            <td class="joined align-middle py-2">{{ local_date($voucher->updated_at, 'd/m/Y H:i') }}</td>
